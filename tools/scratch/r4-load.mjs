...
const r = await p.evaluate(async () => {
  ...
  for (const [k,c,rr] of MAZE) g.build(k,c,rr);
  g.state.phase='combat';
  const c = g.creeps;
  for (let k = 0; k < 30; k++) c.spawn(['normal','fast','armored','swarm','flying'][k%5], 4e7, 5, (k%10)*0.8);
  await wait(2500);
  const set = (v) => { for (const a of Object.values(c.archetypes)) { a.outline.visible=v; a.xray.visible=v; } };
  await wait(300); const i = g.pipeline.renderer.info;
  const all = { calls: i.render.calls, tris: i.render.triangles };
  set(false); await wait(400); const off = { calls: i.render.calls, tris: i.render.triangles }; set(true);
  return { alive: c.count, all, off };
});
await b.close();
